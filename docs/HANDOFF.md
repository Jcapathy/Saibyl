...
| # | Item | Type | Blocking |
|---|---|---|---|
| 1 | **Work down `docs/V1_AUDIT.md`** ← **the work.** The systematic sweep found ~70 findings; 4 are fixed, the rest are ranked and open. Two are security or money and are already done; the highest-value cluster still open is **one dead key with six customer-visible readers** (`metadata.sentiment`, audit item 8). Verify each item before fixing — several are reasoned rather than observed. | Work | Phase 4 |
| 2 | **Phase 4** — Crisis lens, `clients` layer + org switcher, durable background jobs, calibration. Starts on one codebase: the merge and 019 landed 2026-08-04. Audit items 19–22 overlap the durable-jobs work; do them together. | Phase | — |
