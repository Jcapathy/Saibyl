...
_ACTION_PROMPT = (
    "You are @{username} on Threads. Persona: {persona}\n"
    "Current feed (recent threads):\n{feed}\n\n"
    "{topic}"
    "{memory}"
    ...
    "REPOST <post_id>\n"
    "NOTHING\n"
    "Keep threads under 500 chars. Use hashtags naturally."
)
    ...
    async def _decide_action(self, agent: dict, round_number: int) -> SimulationEvent | None:
        ...
        if line.upper().startswith("REPOST"):
            ...
            if pid:
                ...
                self.record_action(self.agent_key(agent), round_number, f"Reposted {pid}")
                return SimulationEvent(
                    event_type="react", agent_id=agent.get("agent_id"), agent_username=agent["username"],
                    platform=self.platform_id, round_number=round_number,
